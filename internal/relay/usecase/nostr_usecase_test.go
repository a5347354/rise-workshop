...
func Test_relayUsecase_ReceiveMessage_Event(t *testing.T) {
	Convey("ReceiveMessage Event", t, func() {
		mockEventStore, usecase, _ := given(t)
...
func Test_relayUsecase_SendMessageToSubscriber(t *testing.T) {
	Convey("SendMessageToSubscriber", t, func() {
		mockStore, usecase, mockDelivery := given(t)
		mockStore.EXPECT().SearchByContent(gomock.Any(), "").Return([]internal.Event{
			{
				ID:        "1111",
				Kind:      1,
				Content:   "Android",
		...
		err := usecase.SendMessageToSubscriber(context.TODO())
		So(err, ShouldBeNil)
	})
}
